...
Adapted to match the logic used in the event viewer, with node and target
coordinates converted to ATLAS-style polar coordinates.

ATLAS convention used here:
...
Target:
- y_vertex = first valid truth vertex in polar coordinates:
    [r, theta, phi]
  with r in meters, angles in radians
...
def _build_vertex_target(vertex_td, idxs):
    ...
    Use the first valid truth vertex, converted to polar coordinates:
      [r, theta, phi]
    with r in meters and angles in radians.
    """
    for i in idxs:
        xs = np.asarray(vertex_td["truthMuonVertexPositionX"][i]).ravel()
        ys = np.asarray(vertex_td["truthMuonVertexPositionY"][i]).ravel()
