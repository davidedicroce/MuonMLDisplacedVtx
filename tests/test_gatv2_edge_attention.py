...
class GATv2EdgeAttentionTest(unittest.TestCase):
    ...
    def test_default_mode_is_gatv2(self):
        model = self.build_model()
        self.assertTrue(any("gatv2_" in key for key in model.state_dict()))

    def test_explicit_legacy_modes_have_no_new_checkpoint_parameters(self):
        for kwargs in (
            {"gatv2_edge_attn": False},
            {"gat_edge_attn": True, "gatv2_edge_attn": False},
        ):
