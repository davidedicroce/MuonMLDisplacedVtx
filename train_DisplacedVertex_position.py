...
def _normalize_path_str(p: str) -> str:
    ...
    symlink spellings, and relative/absolute path usage.
    """
    try:
...
def _check_split_paths_compatible(split_npz, current_paths, *, strict: bool = False):
    """
    Validate that the split file was produced from the same H5 parts, but do it
    robustly enough that moving the training directory or using different relative
    paths does not hard-fail.
...
def global_pool(h: torch.Tensor, mode: str = "meanmax") -> torch.Tensor:
    """
    Pool node embeddings of a single graph into one graph embedding.
...
class DisplacedVertexGNN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, edge_dropout_p: float = 0.0):
        ...
        g = global_pool(h, mode=self.pool)   # [1, G]
...
def estimate_target_stats(train_ds, device, max_events: int = -1):
    """
    Estimate mean/std of y_vertex over the training subset.
    Rank0 computes, then broadcasts.
    """
    if ddp_is_main():
        loader = DataLoader(
            train_ds,
...
def regression_stats(pred: torch.Tensor, target: torch.Tensor):
    ...
    return mae, rmse, mae_x, mae_y, mae_z
...
def main():
    ...
    if ddp_is_main():
        print(f"[i] target_mean={target_mean.detach().cpu().numpy()}", flush=True)
        print(f"[i] target_std ={target_std.detach().cpu().numpy()}", flush=True)
    ...
    for epoch in range(int(start_epoch), args.epochs + 1):
        ...
        train_mae_x = torch.tensor(0.0, device=device)
        train_mae_y = torch.tensor(0.0, device=device)
        ...
        for batch in train_loader:
            ...
            with torch.no_grad():
                ...
                mae, rmse, mae_x, mae_y, mae_z = regression_stats(pred_metric, y)
        ...
        ddp_all_reduce_sum(train_mae_x)
        ddp_all_reduce_sum(train_mae_y)
        ...
        train_mae_x_mean = (train_mae_x / torch.clamp(train_steps, min=1.0)).item()
        train_mae_y_mean = (train_mae_y / torch.clamp(train_steps, min=1.0)).item()
        ...
        val_mae_x = torch.tensor(0.0, device=device)
        val_mae_y = torch.tensor(0.0, device=device)
        ...
        ddp_all_reduce_sum(val_mae_x)
        ddp_all_reduce_sum(val_mae_y)
        ...
        val_mae_x_mean = (val_mae_x / torch.clamp(val_steps, min=1.0)).item()
        val_mae_y_mean = (val_mae_y / torch.clamp(val_steps, min=1.0)).item()
        ...
        if ddp_is_main():
            print(
                ...
                f"(x={train_mae_x_mean:.5f}, y={train_mae_y_mean:.5f}, z={train_mae_z_mean:.5f}) | "
                f"val loss={val_loss_mean:.5f} mae={val_mae_mean:.5f} rmse={val_rmse_mean:.5f} "
                f"(x={val_mae_x_mean:.5f}, y={val_mae_y_mean:.5f}, z={val_mae_z_mean:.5f}) | "
                f"lr={current_lr:.3e} | "
            ...
            wandb.log(
                {
                    ...
                    "train/mae_x": train_mae_x_mean,
                    "train/mae_y": train_mae_y_mean,
                    "train/mae_z": train_mae_z_mean,
                    "val/loss": val_loss_mean,
                    "val/mae": val_mae_mean,
                    "val/rmse": val_rmse_mean,
                    "val/mae_x": val_mae_x_mean,
                    "val/mae_y": val_mae_y_mean,
