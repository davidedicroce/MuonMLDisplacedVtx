...
torchrun --standalone --nproc_per_node=8 train_DisplacedVertex_cylindrical_position.py \
    ...
    --ema \
    --ema-decay 0.999 \
    --save "./models/displaced_vertex_cylindrical_gnn.pt"
"""
...
class H5CylindricalEventDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if x.shape[1] != 7:
            raise RuntimeError(
                f"Expected 7 node features for cylindrical-target dataset, got {x.shape[1]} "
                f"in {self.h5_paths[fi]} /events/{k}"
            )

        if y_vertex.numel() != 3:
            raise RuntimeError(
                f"Expected y_vertex = [rho, phi, z], got shape {tuple(y_vertex.shape)} "
                f"in {self.h5_paths[fi]} /events/{k}"
            )
...
def main():
    ...
    ap.add_argument("--save", default="displaced_vertex_cylindrical_gnn.pt")
    ...
    if "coordinate_system" in split.files:
        ...
        if coord.lower() != "cylindrical":
            raise RuntimeError(
                f"Split file indicates coordinate_system={coord!r}, expected 'cylindrical'."
            )
    ...
    if ddp_is_main():
        print(f"[i] target_mean={target_mean.detach().cpu().numpy()}  # [rho, phi, z]", flush=True)
        print(f"[i] target_std ={target_std.detach().cpu().numpy()}  # [rho, phi, z]", flush=True)
