...
"""
Prepare and save a deterministic train/val split for the displaced vertex dataset.
...
python DisplacedVertex_splitter.py \
  ...
  --val-fraction 0.1 \
  --seed 12345 \
  --out split_displaced_vertex_seed12345.npz

Later in training:
  split = np.load("split_displaced_vertex_seed12345.npz", allow_pickle=True)
...
# ------------------------------------------------------------
# Dataset loader (matches DisplacedVertex_converter output)
# ------------------------------------------------------------

class H5EventDataset(Dataset):
    """
    Dataset for displaced vertex graph data.
...
def main():
    ...
    ap.add_argument(
        "--data-glob",
        required=True,
        help='Glob for H5 files, e.g. "./data/displaced_vertex_dataset_part*.h5"',
    )
