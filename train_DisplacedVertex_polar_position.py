...
class H5PolarEventDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if x.ndim != 2:
            raise RuntimeError(
                f"Expected x with shape [N, F], got {tuple(x.shape)} in {self.h5_paths[fi]} /events/{k}"
            )

        if x.shape[1] != 7:
            raise RuntimeError(
                f"Expected 7 node features for polar dataset, got {x.shape[1]} "
                f"in {self.h5_paths[fi]} /events/{k}"
            )

        if y_vertex.numel() != 3:
            raise RuntimeError(
                f"Expected y_vertex = [r, theta, phi], got shape {tuple(y_vertex.shape)} "
                f"in {self.h5_paths[fi]} /events/{k}"
            )
    ...
    def __init__(
        ...
        if use_fourier:
            self.fourier = FourierEncoder(
                xdim,
                base=fourier_base,
                min_exp=fourier_min_exp,
                max_exp=fourier_max_exp,
            )
...
def regression_stats(pred: torch.Tensor, target: torch.Tensor):
    ...
    mae_r = abs_diff[0]
    mae_theta = abs_diff[1]
    mae_phi = abs_diff[2]

    return mae, rmse, mae_r, mae_theta, mae_phi
...
def main():
    ...
    ap.add_argument("--strict-split-check", action="store_true", default=False,
                    help="Require exact split-file path compatibility. By default, path checking is relaxed.")
    ...
    ap.add_argument("--target-stats-max-events", type=int, default=-1,
                    help="Use only first N train events to estimate target mean/std; -1 = all.")

    ap.add_argument("--max-train-events", type=int, default=-1)

    ap.add_argument("--save", default="displaced_vertex_polar_gnn.pt")
    ...
    if "coordinate_system" in split.files:
        ...
        if coord.lower() != "polar":
            raise RuntimeError(
                f"Split file indicates coordinate_system={coord!r}, expected 'polar'."
            )
    ...
    if ddp_is_main():
        print(f"[i] target_mean={target_mean.detach().cpu().numpy()}  # [r, theta, phi]", flush=True)
        print(f"[i] target_std ={target_std.detach().cpu().numpy()}  # [r, theta, phi]", flush=True)
    ...
    for epoch in range(int(start_epoch), args.epochs + 1):
        ...
        train_mae_r = torch.tensor(0.0, device=device)
        train_mae_theta = torch.tensor(0.0, device=device)
        ...
        for batch in train_loader:
            ...
            with torch.no_grad():
                ...
                mae, rmse, mae_r, mae_theta, mae_phi = regression_stats(pred_metric, y)
        ...
        ddp_all_reduce_sum(train_mae_r)
        ddp_all_reduce_sum(train_mae_theta)
        ...
        train_mae_r_mean = (train_mae_r / torch.clamp(train_steps, min=1.0)).item()
        train_mae_theta_mean = (train_mae_theta / torch.clamp(train_steps, min=1.0)).item()
        ...
        val_mae_r = torch.tensor(0.0, device=device)
        val_mae_theta = torch.tensor(0.0, device=device)
        ...
        ddp_all_reduce_sum(val_mae_r)
        ddp_all_reduce_sum(val_mae_theta)
        ...
        val_mae_r_mean = (val_mae_r / torch.clamp(val_steps, min=1.0)).item()
        val_mae_theta_mean = (val_mae_theta / torch.clamp(val_steps, min=1.0)).item()
        ...
        if ddp_is_main():
            print(
                ...
                f"(r={train_mae_r_mean:.5f}, theta={train_mae_theta_mean:.5f}, phi={train_mae_phi_mean:.5f}) | "
                f"val loss={val_loss_mean:.5f} mae={val_mae_mean:.5f} rmse={val_rmse_mean:.5f} "
                f"(r={val_mae_r_mean:.5f}, theta={val_mae_theta_mean:.5f}, phi={val_mae_phi_mean:.5f}) | "
                f"lr={current_lr:.3e} | "
            ...
            wandb.log(
                {
                    ...
                    "train/mae_r": train_mae_r_mean,
                    "train/mae_theta": train_mae_theta_mean,
                    "train/mae_phi": train_mae_phi_mean,
                    "val/loss": val_loss_mean,
                    "val/mae": val_mae_mean,
                    "val/rmse": val_rmse_mean,
                    "val/mae_r": val_mae_r_mean,
                    "val/mae_theta": val_mae_theta_mean,
