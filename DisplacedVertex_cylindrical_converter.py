...
Adapted to match the logic used in the event viewer, with node coordinates
converted to ATLAS-style polar coordinates, but with the target vertex stored
in cylindrical coordinates.

ATLAS convention used here for node features:
- phi   = atan2(y, x)                       (angle in the x-y plane)
- theta = atan2(rho, z), rho = sqrt(x^2+y^2)
          i.e. angle with respect to the z axis

Event selection:
- keep only events where ALL truth vertices are inside the envelope
    r_xy <= --vertex-r-max-mm
    |z|   <= --vertex-z-max-mm
...
Target:
- y_vertex = first valid truth vertex in cylindrical coordinates:
    [rho, phi, z]
  with rho in meters, phi in radians, z in meters
...
where:
  - for muon nodes:
      r, theta_pos, phi_pos = segment position converted from Cartesian [m]
      theta_dir, phi_dir    = segment direction converted from Cartesian
      energy_like           = 0
      nCells_or_DoF         = segment_numberDoF

  - for calo nodes:
      r, theta_pos, phi_pos = tower calorimeter intersection point [m]
      theta_dir, phi_dir    = tower direction converted from Cartesian
...
def _cartesian_to_atlas_position_polar(x, y, z):
    """
    Convert Cartesian position to ATLAS-style spherical coordinates.
    ...
    Input/output units:
      - x, y, z keep their input length unit
...
def _cartesian_to_cylindrical(x, y, z):
    """
    Convert Cartesian position to cylindrical coordinates.
...
def _cartesian_to_atlas_direction_angles(dx, dy, dz):
    """
    Convert Cartesian direction vector to ATLAS-style angular coordinates.
...
def _event_passes_vertex_envelope(vertex_td, idxs, vertex_r_max_mm, vertex_z_max_mm):
    ...
    inside = (r_xy <= vertex_r_max_mm) & (np.abs(z) <= vertex_z_max_mm)
...
def _build_vertex_target(vertex_td, idxs):
    ...
    Use the first valid truth vertex, converted to cylindrical coordinates:
      [rho, phi, z]
    with rho in meters, phi in radians, z in meters.
    """
    for i in idxs:
...
def _build_muon_nodes(td, idxs):
    ...
    theta_dir, phi_dir, dir_u = _cartesian_to_atlas_direction_angles(dx, dy, dz)
...
def edge_features(energy_like, phi, eta, dir_u, sector, node_type, edge_index):
    ...
    return np.concatenate(
        [d_energy_like, d_phi, d_eta, cosang, same_sector],
        axis=1,
    ).astype(np.float32)
